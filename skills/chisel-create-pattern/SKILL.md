...
6. **Ensure `patterns` is imported** — on a fresh project the `patterns/` layer may not be wired up yet. Verify `@use 'patterns';` exists in **both** `src/styles/main.scss` and `src/styles/editor.scss`, added **after** `@use 'components';`. Add it if missing (this is a one-time per-project step).
