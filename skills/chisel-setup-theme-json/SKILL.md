...
7. `settings.custom.spacer` — the `core/spacer` scale, and **its own set of values**: `.is-style-X` renders exactly `settings.custom.spacer.X`, nothing else feeds it. This step decides what spacer options editors get, so pull the spec's section-gap values here, not just the component ones. These are fluid by default — interpolate between the same viewport anchors as `settings.typography.fluid`; shape and formula in [theme-json-recipes.md "Spacer size"](.claude/chisel/templates/theme-json-recipes.md#spacer-size). Renaming or adding an alias means updating `$_spacer-sizes` in `src/styles/blocks/_core-spacer.scss` and `registerSpacerStyles()` in `src/scripts/editor/blocks-styles.js` in the same change. **Changing a value also means updating the label** — each picker label embeds its range (`Large (2.5-4rem)`), so a resized alias with a stale label lies to editors.
8. `settings.custom.*` — the remaining categories after steps 6 and 7: `border-radius`, `border-width`, `box-shadow`, `letter-spacing`, `line-height`, `transition`. Ten categories ship in total; read `settings.custom` and account for each one rather than working from this list
9. `settings.layout` — `contentSize` / `wideSize`. These drive the page rail, so both widths move every block's edges at once ([blocks.md "Wide and full width"](.claude/chisel/reference/blocks.md#wide-and-full-width))
10. `styles.elements.h1..h6` — if the heading scale shifted
...
Re-point `chisel_preload_fonts` at the new body face — the default preloads `roboto-regular.woff2` and silently skips a missing file: [assets-and-scripts.md "Default assets"](.claude/chisel/reference/assets-and-scripts.md#default-assets).
