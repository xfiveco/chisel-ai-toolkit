...
7. `settings.custom.*` — the remaining categories after step 6's `margin`/`padding`/`gap`: `border-radius`, `border-width`, `box-shadow`, `letter-spacing`, `line-height`, `transition`. Nine categories ship in total; read `settings.custom` and account for each one rather than working from this list
