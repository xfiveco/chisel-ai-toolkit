---
name: chisel-resume
description: Pick up an in-flight Chisel task in a fresh session — read the progress files, state where the work stands, and open the next phase. Use when the user says "resume", "continue", "what's next", "where were we", or names a task folder. Do NOT use to start new work (that's /chisel-new-task) or to answer questions about existing code.
argument-hint: "[NN-task-name]"
allowed-tools:
  - Read
  - Edit
  - Glob
...
Read `{{THEME_ROOT}}/ai-progress/INDEX.md`.
...
- **No argument, one task Active** — use it.
- **No argument, several Active** — list them with their one-phrase states and ask which. Don't
  guess.
- **Nothing Active** — say so, mention the most recent `## Done` task in case they meant that, and
  point at `/chisel-new-task`.

## 2. Reload state

Open `changes/{NN}-{task-name}/ROADMAP.md` and read `**Status:**`, `## Source`, `## Scope`,
`## Decisions`, and the phase table.

The `[x]` rows' one-line outcomes are what happened — **don't open completed phase files.** If a
row is `[x]` with no outcome, that's a gap: note it, and reconstruct from `LOG.md` rather than
reopening the phase file.

Then find the current phase — the first row that is `[~]`, or the first `[ ]` after the last `[x]`
— and open **only that** `phase-NN-*.md`. Read `LOG.md` only if the roadmap leaves you unsure where
the trail ends.
...
- Where the task stands — status, phases done, what's next.
...
Then ask whether to start that phase. **Stop.** Starting it means the plan-review gate in
`/chisel-new-task` step 4 — flip the row to `[~]`, fill in the checklist, pause again. Don't skip
ahead to code because the plan was already written.

## Notes

- Blocked (`[!]`) phase and the user wants to move? Offer the next unblocked phase rather than
  forcing order — then note the reorder in `LOG.md`.
...
- If the files and your recollection disagree, the files win. Full procedure: `/chisel-plan`.
